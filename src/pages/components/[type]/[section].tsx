...
export const SectionTabs: FC<{ onSelectTab: (tab: string) => void }> = ({ onSelectTab }) => {
  return (
    <section className='p-4 px-20 bg-[#2b3135] border-b-gray-600'>
      <Link href={`/components/marketing/all`}>
        <span className='pr-6 text-md font-bold hover:text-[#fc8936] text-gray-300 cursor-pointer' onClick={() => onSelectTab("marketing")}>
          Marketing
        </span>
      </Link>
      <Link href={`/components/applicationUI/all`}>
        <span
          className='pr-6 text-md font-bold hover:text-[#fc8936] text-gray-300 cursor-pointer'
          onClick={() => onSelectTab("applicationUI")}>
          Application UI
        </span>
      </Link>
      <Link href={`/components/ecommerce/all`}>
        <span className='pr-6 text-md font-bold hover:text-[#fc8936] text-gray-300 cursor-pointer' onClick={() => onSelectTab("ecommerce")}>
          Ecommerce
        </span>
      </Link>
    </section>
...
interface ISectionTabComponents {
  [type: string]: {
    title: string;
    desc: string;
    componentType: { name: string; typeId: string }[];
  };
}

const sectionTabComponent: ISectionTabComponents = {
  marketing: {
    title: "Marketing",
    desc: "Heroes, feature sections, newsletter sign up forms — everything you need to build beautiful marketing websites.",
    componentType: [
      { name: "Page Sections", typeId: "page_sections" },
      { name: "Element", typeId: "element" },
      { name: "Feedback", typeId: "feedback" },
      { name: "Page Example", typeId: "page_example" },
    ],
  },
  applicationUI: {
  ...
  ecommerce: {
    title: "Ecommerce",
    desc: "Checkout forms, shopping carts, product views — everything you need to build your next ecommerce front-end.",
    componentType: [],
  },
};

export default function Home() {
  const [tabInfo, setTabInfo] = useState<{ title: string; desc: string; componentType: { name: string; typeId: string }[] }>({
    title: sectionTabComponent["marketing"].title,
    desc: sectionTabComponent["marketing"].desc,
    componentType: sectionTabComponent["marketing"].componentType,
  });

  const onSelectTab = (tabName: string) => {
    setTabInfo(sectionTabComponent[tabName]);
  };
  return (
    <>
      <Head>
      ...
      <main>
        <Navigation />
        <HeroSection />
        <SectionTabs onSelectTab={onSelectTab} />
        <ComponentsSection title={tabInfo.title} desc={tabInfo.desc} componentType={tabInfo.componentType} />
      </main>
    </>
  );
}
